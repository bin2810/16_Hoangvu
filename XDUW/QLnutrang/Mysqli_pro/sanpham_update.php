...
if(isset($_GET['id'])){
    ...
    $sql_sp = "SELECT * FROM tbsanpham WHERE SanPham_id = ".$SanPham_id;
    ...
    $lsp = mysqli_query($conn, $sql_lsp);
    $sp = mysqli_query($conn, $sql_sp);

    if(mysqli_num_rows($lsp) > 0){
        $loaisp = mysqli_fetch_all($lsp, MYSQLI_ASSOC);
    }
    if(mysqli_num_rows($sp) > 0){
        $sanpham = mysqli_fetch_object($sp);
    }
?>
<body>
   <div class="yeucau">
