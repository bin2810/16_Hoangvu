<?php
    if (isset($_POST['ThemMoi'])) {
        ...
        $query_nameloai = "SELECT * FROM tbloaisanpham WHERE TenLoai = '" . $tenloai . "'";
        $kt_nameloai = $conn->query($query_nameloai);
        if($kt_nameloai->num_rows > 0){
            echo"trùng tên sản phẩm";
        }else{
            $query_maloai = "SELECT * FROM tbloaisanpham WHERE MaLoai = '" . $maloai . "'";
            $kt_maloai = $conn->query($query_maloai);
            if($kt_maloai->num_rows > 0){
                echo"trùng mã loại";
            }else{
                $sql = "INSERT INTO tbloaisanpham (`MaLoai`, `TenLoai`, `HinhAnh`) VALUES (?, ?, ?)";
